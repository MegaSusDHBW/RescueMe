...
def encrypt():
    ...
    results = db.session.query(User.User, HealthData.HealthData, EmergencyContact.EmergencyContact).join(HealthData.HealthData).join(EmergencyContact.EmergencyContact).filter(User.User.email == user_mail).all()
    print(str(results))
